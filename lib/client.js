...
class Client extends EventEmitter {
	...
	_sendMessage(delay, channel, message, fn, reply_id) {
		// Promise a result..
		return new Promise((resolve, reject) => {
			...
			if(message.length >= 500) {
				...
				setTimeout(() => {
					this._sendMessage(delay, channel, msg[1], () => {}, reply_id);
				}, 350);
			}

			this.ws.send((reply_id ? `@reply-parent-msg-id=${reply_id} ` : '') + `PRIVMSG ${chan} :${message}`);
